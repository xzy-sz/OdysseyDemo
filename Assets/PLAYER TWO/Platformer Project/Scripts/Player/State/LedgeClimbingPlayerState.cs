...
namespace Assets.PLAYER_TWO.Platformer_Project.Scripts.PlayerLib.State
{
    ...
    public class LedgeClimbingPlayerState : PlayerState
    {
        ...
        protected virtual IEnumerator SetPositionRoutine(Player player)
        {
            ...
            var halfDuration = totalDuration / 2f;                          // 分段总时长

            var initialPosition = player.transform.localPosition;           // 初始位置
            var targetVerticalPosition = player.transform.position + Vector3.up * 
                                         (player.height + Physics.defaultContactOffset);
            ...
            while(elapsedTime <= halfDuration)
            {
                ...
                player.transform.localPosition = Vector3.Lerp(initialPosition, targetVerticalPosition,
            ...
            {
                elapsedTime += Time.deltaTime;
                player.transform.localPosition = Vector3.Lerp(targetVerticalPosition,targetLateralPosition,
                                                              elapsedTime / halfDuration);
                yield return null;
            }
