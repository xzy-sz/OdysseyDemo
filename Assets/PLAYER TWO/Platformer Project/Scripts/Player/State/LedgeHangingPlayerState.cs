...
namespace Assets.PLAYER_TWO.Platformer_Project.Scripts.PlayerLib.State
{
    ...
    public class LedgeHangingPlayerState : PlayerState
    {
        ...
        /// - 若满足悬挂条件：约束玩家位置与朝向，并处理左右移动，释放与跳跃
        /// -若条件不满足：切换为下落状态。
        /// </summary>
        /// <param name="player"></param>
        ...
        {
            // ----------- 顶端（平台面）检测参数 -----------
            // 以玩家半径 + 可向前探测的最大距离作为”顶端检测“的向前探测范围
                    ...
                    QueryTriggerInteraction.Ignore))
            {
                ...
                // 计算玩家悬挂时应该放置的"竖直高度"
                // 取平台命中点的y,再及拿去玩家半高(让玩家胸口附近对齐到平台边缘下方)
                ...
                var destinationHeight = player.height * 0.3f + Physics.defaultContactOffset;

                // 计算翻越时的目标落点位置
                // 平台命中点 + 上拾到能容纳玩家胶囊的高度 + 向前挪一个半径（避免卡台边）
                ...
                // 若打空，说明边缘到了（旁边没有墙可挂），不允许继续横移，置零速度
                if(Physics.Raycast(
                    ledgeSideOrigin,
                    sideForward,
                    rayDistance,
                    ...
                    QueryTriggerInteraction.Ignore))
                {
                    // 允许横移：按输入 x 方向设置为横向速度(世界空间的左右 = transform.right)
                ...
                // 再沿"面向墙的方向"后退一个半径(让玩家胶囊体刚好贴在墙外缘)
                ...
                // ------------- 输入处理：释放 / 跳跃 / 翻越 -------------
                if (player.inputs.GetReleaseLedgeDown()) // 1.释放悬挂：玩家松手，切回下落
                {
                    // 松手时把朝向反过来（面朝离开方向，可选的表现细节）
                    player.FaceDirection(-sideForward);
                    player.states.Change<FallPlayerState>();
                }
                else if(player.inputs.GetJumpDown()) // 2.跳跃：从悬挂状态直接跳起，然后切回下落
                {
