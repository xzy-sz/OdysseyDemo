...
{
    // 依赖要求：挂宅该脚本的物体必须有Collider组件
    [RequireComponent(typeof(Collider))]
    // 在 Unity 组件菜单为该类添加路径，方便在编辑器中添加
    ...
    public class EntityHitbox : MonoBehaviour
    {
        ...
        protected Collider m_collider; // 缓存自身碰撞器的引用

        /// <summary>
        /// Untiy 生命周期方法，脚本启用时调用一次
        /// </summary>
        protected virtual void Start()
        {
            InitializeEntity(); // 初始化实体引用
            InitializeCollider(); // 初始化碰撞器引用
        }
        ...
        {
            // 调用对方的 AppleDamage 方法，传入伤害值和攻击位置
